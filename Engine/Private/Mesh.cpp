...
#pragma region INDEX_BUFFER
	...
	{
		m_pIndices[index++] = _pAIMesh->mFaces[i].mIndices[0];
		m_pIndices[index++] = _pAIMesh->mFaces[i].mIndices[1];
		m_pIndices[index++] = _pAIMesh->mFaces[i].mIndices[2];
	}
...
HRESULT CMesh::Ready_VertexBuffer_NonAnim(const aiMesh* _pAIMesh)
{
	...
	for (size_t i = 0; i < m_iNumVertices; ++i)
	{
		...
		pVertices[i].vPosition	= _float3(XMVector3TransformCoord(_float3(pVertices[i].vPosition), m_mPivot));
